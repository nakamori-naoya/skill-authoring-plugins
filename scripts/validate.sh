...
if jq -e '.name=="skill-authoring" and .version=="0.1.1" and .skills==["./skills/author-skill"] and .interface.capabilities==["Skills"] and .metadata.harness=={"marketplace":"skill-authoring","contractVersion":1}' "$PLUGIN/.codex-plugin/plugin.json" >/dev/null \
  && jq -e '.name=="skill-authoring" and .version=="0.1.1" and .skills==["./skills/author-skill"] and .metadata.harness=={"marketplace":"skill-authoring","contractVersion":1}' "$PLUGIN/.claude-plugin/plugin.json" >/dev/null; then

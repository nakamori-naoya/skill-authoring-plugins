...
skill本体は、主要な利用場面で責務と判断基準が機能するか、目的・入力・判断基準・手順・停止条件・出力が本文に揃っているか、否定文の連打や配管の説明が残っていないか、停止条件が止まる条件（必須入力の欠落・公開契約違反・許可が要る変更・toolの失敗・保存先が確認できない）と仮説を明示して進む条件に分かれているか、問う場面が1回の実行で最大6問と推奨の仮置きの規律を持つか、妥当な裁量を定型へ押し込めていないかを目視確認します。この意味評価を語句の存在検査や文長で代理しません。
